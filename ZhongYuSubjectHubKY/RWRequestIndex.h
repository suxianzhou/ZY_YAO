//  Copyright © 2016年 RyeWhiskey. All rights reserved.
//

#ifndef RWRequestIndex_h
#define RWRequestIndex_h

#define UMengCommunityAppkey @"5762610b67e58eea66001c6b"
#define UMengCommunityAppSecret @"9cedab0b0cc8097773fe3039ec8426ca"

#define MOB_CLICK @"57316e8167e58e28c4000286"

#define NAV_TITLE @"药师资讯"

#define MAIN_INDEX [NSURL URLWithString:@"http://m.zhongyuyao.com/list.php?fid=55"]
//>> 资讯
#define SERVERS_INDEX @"http://www.zhongyuedu.com/api/tk_yao_examtype.php"
//>> 题库目录
#define YY_INDEX @"http://api.zhongyuedu.com/tik/yao/yy_yao_new_list.php"
//>> 预约列表
#define RECOMMEND @"http://www.zhongyuedu.com/api/tuijian.php"
//>> 推荐列表
#define UPDATE_USERNAME @"http://www.zhongyuedu.com/api/yy_yao_post.php"
//>> 上传
#define EXPERIENCE_TIMES_URL @"http://api.zhongyuedu.com/tik/yao/yao_limit.php"
//>> 天王盖地虎
#define RECEIVE_PUSH @"http://www.zhongyuedu.com/api/yao_url.php"
//>> push
#define LOGIN_URL @"http://api.zhongyuedu.com/tik/yao/login.php"
//>> 登录
#define REGISTER_URL @"http://api.zhongyuedu.com/tik/yao/tk_yao_register.php"
//>> 注册
#define REPLACE_PASSWORD_URL @"http://api.zhongyuedu.com/tik/yao/yao_change_pwd.php"
//>> 重置密码
#define ABOUT_US @"http://www.zhongyuedu.com/api/tk_aboutUs.htm"
//>> 关于我们
#define README @"http://www.zhongyuedu.com/api/tk_yao/tk_ios_usage.htm"
//>> 使用说明
#define VERIFICATION_PHONENUMBER @"http://api.zhongyuedu.com/comm/code.php"

#endif /* RWRequestIndex_h */
